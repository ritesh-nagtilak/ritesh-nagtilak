<h1 align="center">Hi 👋, I'm Ritesh Nagtilak</h1><h3 align="center">Oracle Technical Consultant • Oracle Fusion Cloud • Oracle Integration Cloud (OIC) • Python Developer</h3><p align="center">
Passionate about building enterprise solutions, automating business processes, and continuously learning new technologies.
</p>

👨‍💻 About Me

- 💼 Oracle Technical Consultant at AAIS Global
- ☁️ Working on Oracle Fusion Cloud (ERP & HCM)
- 🔄 Developing integrations using Oracle Integration Cloud (OIC)
- 📊 Building BI Publisher Reports & Data Models
- 🔌 Working with REST APIs, SOAP Services, SQL & PL/SQL
- 🐍 Python enthusiast with a passion for automation
- 🤖 Currently learning Generative AI & Large Language Models
- ☁️ Microsoft Azure Fundamentals (AZ-900) Certified
- 🏍️ Love bike rides and exploring new places
- 🌱 Always learning and building

---

🛠️ Tech Stack

Oracle

- Oracle Fusion Cloud
- Oracle Integration Cloud (OIC)
- BI Publisher
- Oracle SQL
- PL/SQL
- REST APIs
- SOAP Web Services
- FBDI

Languages & Tools

<p align="left">
<img src="https://skillicons.dev/icons?i=python,js,html,css,git,github,flask,fastapi,azure,vscode,postman,mysql" />
</p>---

🚀 Current Focus

- Oracle Fusion Cloud Development
- Oracle Integration Cloud (OIC)
- Enterprise Integrations
- BI Publisher Reporting
- REST API Development
- Python Automation
- Artificial Intelligence

---

🌱 Currently Learning

- Generative AI
- Large Language Models (LLMs)
- Oracle AI Services
- Cloud Architecture
- Advanced Integration Patterns

---

📂 Featured Projects

- 🔹 Oracle Fusion Cloud Integrations
- 🔹 Oracle Integration Cloud (OIC) Solutions
- 🔹 BI Publisher Reports
- 🔹 Python Automation Scripts
- 🔹 REST API Integrations
- 🔹 AI & Machine Learning Projects

---

📊 GitHub Stats

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=ritesh-nagtilak&show_icons=true&theme=tokyonight&hide_border=true"/>
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=ritesh-nagtilak&theme=tokyonight&hide_border=true"/>
</p><p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ritesh-nagtilak&layout=compact&theme=tokyonight&hide_border=true"/>
</p>---

🤝 Let's Connect

I'm always open to collaborating on:

- Oracle Fusion Cloud
- Oracle Integration Cloud (OIC)
- Python Projects
- Artificial Intelligence
- Enterprise Automation
- Open Source

---

💭 Quote

«"Technology creates the greatest impact when it simplifies complexity."»

<p align="center">
⭐ Thanks for visiting my profile! If you like my work, consider giving a ⭐ to my repositories.
</p>